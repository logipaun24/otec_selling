...
class OTECQuotation(Document):
	...
	def set_item_master_values(self):
		...
		for row in self.items:
			...
			_apply_specification_pricing(row)
	...
	def calculate_totals(self):
		...
		for category, rows in category_rows.items():
			...
			for row in rows:
				...
				row.amount = (
					row.base_line_amount
					+ row.allocated_sqm_amount
					+ row.operable_amount
					+ row.allocated_markup
					+ row.addon_amount
				)
	...
	def _validate_and_price_addons(self):
		...
		for addon_row in self.quotation_addons:
			...
			if not item_row:
				frappe.throw(
					_("Add-on {0} is not linked to a valid quotation item row.").format(addon_row.add_on)
				)
			if addon_row.add_on in selected_by_row[item_row.name]:
				frappe.throw(
					_("Add-on {0} is selected more than once for row {1}.").format(
						addon_row.add_on, item_row.idx
					)
				)
			...
			if not rule:
				frappe.throw(
					_("Add-on {0} is not allowed for item {1}.").format(addon_row.add_on, item_row.item_code)
				)
			...
			summaries[item_row.name].append(f"{addon_row.add_on} x {addon_row.quantity:g}")
			if rule.requires_approval:
				item_row.pricing_approval_required = 1
				item_row.pricing_approval_reason = _append_reason(
					item_row.pricing_approval_reason, rule.notes or _("Add-on pricing requires approval.")
				)
		...
		for item_row in self.items:
			...
			for addon_row in [a for a in self.quotation_addons if a.quotation_item_row_id == item_row.name]:
				...
				if conflicts:
					frappe.throw(
						_("Add-on {0} conflicts with {1} on row {2}.").format(
							addon_row.add_on, ", ".join(sorted(conflicts)), item_row.idx
						)
					)
...
def _active_configurations(item_code):
	...
	for row in active_rows:
		row.base_product_sqm_rate = flt(row.sqm_rate)
	return active_rows
...
SPECIFICATIONS = (
	(
		...
		"OTEC Item Aluminum Option",
		"aluminum_thickness",
		"aluminum_price_per_sqm",
		_("aluminum thickness"),
	),
	(
		"glass_type",
		"OTEC Glass Type",
		"OTEC Item Glass Type Option",
		"glass_type",
...
def _apply_specification_pricing(row):
	component_rates = []
	for row_field, master_doctype, option_doctype, option_field, rate_field, label in SPECIFICATIONS:
		selected = row.get(row_field)
		if not selected:
			frappe.throw(_("Select {0} for quotation row {1}.").format(label, row.idx))
		if not frappe.db.exists(
			option_doctype,
			{"parent": row.item_code, "parenttype": "Item", option_field: selected},
		):
			frappe.throw(_("{0} is not allowed for item {1}.").format(selected, row.item_code))
		master = frappe.db.get_value(
			master_doctype,
			selected,
			["rate_per_sqm", "active", "requires_approval", "approval_reason"],
			as_dict=True,
		)
		if not master or not master.active:
			frappe.throw(_("Selected {0} {1} is inactive or missing.").format(label, selected))
		rate = flt(master.rate_per_sqm)
		row.set(rate_field, rate)
		component_rates.append(rate)
		if master.requires_approval:
			row.pricing_approval_required = 1
			row.pricing_approval_reason = _append_reason(
				row.pricing_approval_reason,
				master.approval_reason or _("{0} pricing requires approval.").format(selected),
			)
	row.sqm_rate = _effective_specification_rate(row.base_product_sqm_rate, *component_rates)


def _effective_specification_rate(base_rate, aluminum_rate, glass_type_rate, glass_color_rate):
	return sum(flt(rate) for rate in (base_rate, aluminum_rate, glass_type_rate, glass_color_rate))
...
def get_product_configurator(item_code):
	...
	return {
		...
		"aluminum_options": _allowed_specification_options(
			item_code, "OTEC Item Aluminum Option", "aluminum_thickness", "OTEC Aluminum Thickness"
		),
		"glass_type_options": _allowed_specification_options(
			item_code, "OTEC Item Glass Type Option", "glass_type", "OTEC Glass Type"
		),
		"glass_color_options": _allowed_specification_options(
			item_code, "OTEC Item Glass Color Option", "glass_color", "OTEC Glass Color"
		),
		"add_on_rules": rules,
	}


def _allowed_specification_options(item_code, child_doctype, link_field, master_doctype):
	...
	for name in names:
		master = frappe.db.get_value(
			master_doctype,
			name,
			["rate_per_sqm", "requires_approval", "approval_reason", "active"],
			as_dict=True,
		)
		if master and master.active:
			options.append(
				frappe._dict(
					name=name,
					rate_per_sqm=flt(master.rate_per_sqm),
					requires_approval=master.requires_approval,
					approval_reason=master.approval_reason,
				)
			)
	return options
